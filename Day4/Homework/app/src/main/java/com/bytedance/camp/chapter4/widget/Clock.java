...
import java.util.concurrent.Executor;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        private void drawTimeNeedles(Canvas canvas) {
            ...
            double secondRad = 2 * Math.PI * second / 60.0;
            double minuteRad = 2 * Math.PI * (minute * 60 + second) / (60.0 * 60);
            double hourRad = 2 * Math.PI * ((hour % 12) * 3600 + minute * 60 + second) / (60.0 * 60 * 12);
            ...
            canvas.drawLine(centerX, centerY,
                    (float) (centerX + HOUR_NEEDLE_LENGTH_RATIO * radius * Math.cos(hourRad)),
                    (float) (centerY + HOUR_NEEDLE_LENGTH_RATIO * radius * Math.sin(hourRad)),
                    needlePaint);

            needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
            canvas.drawLine(centerX, centerY,
                    (float) (centerX + MINUTE_NEEDLE_LENGTH_RATIO * radius * Math.cos(minuteRad)),
                    (float) (centerY + MINUTE_NEEDLE_LENGTH_RATIO * radius * Math.sin(minuteRad)),
                    needlePaint);

            needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
            canvas.drawLine(centerX, centerY,
                    (float) (centerX + SECOND_NEEDLE_LENGTH_RATIO * radius * Math.cos(secondRad)),
                    (float) (centerY + SECOND_NEEDLE_LENGTH_RATIO * radius * Math.sin(secondRad)),
                    needlePaint);
        }
